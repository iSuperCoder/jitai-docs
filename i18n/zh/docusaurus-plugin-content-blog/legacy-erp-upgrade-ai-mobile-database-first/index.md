...
这时平台化会更省力。在 JitAI 里，团队通常会把数据库表导入为具备 ORM 特性的模型，移动端 UI 与智能体工具调用复用同一套模型；审批与审计作为执行层能力接入后，端到端链路更容易看清并治理。如果你想看一个具体的上手流程，可以从 [JitAI 教程](https://jit.pro/zh/docs/devguide) 开始。
