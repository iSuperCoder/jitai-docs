...
| 分组 | 主要配置 |
| --- | --- |
| 基础配置 | 提示词、LLM 配置、输入 & 输出、欢迎语 & 预置问题 |
| 能力编排 | Skills、Functions、数据模型、知识库，DeepAgent 还包含子 Agent |
| 运行与安全 | 上下文、记忆存储、隐私保护、文件空间、联网搜索、工作页面、运行扩展 |

<!-- TODO: 配图 - 截图：编辑器双模式概览（配置面板和代码面板并排） -->

## 选择大模型

大模型是 Agent 智能决策和任务执行的核心。在 `基础配置` -> `LLM配置` 中完成以下设置：
...
- [为 Agent 开启联网搜索](./agent-web-search)，查询公开网页资料
