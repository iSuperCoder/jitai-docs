...
| 想在 IDE 中用自然语言创建或修改 JitAI 元素 | AI 开发助手 | 生成可审阅、可构建验证的元素变更 |
| 想回答一个开放的经营或运营问题 | 业务数据洞察分析 | 得到分析报告 |

如果从能力边界来看，这些 Agent 其实可以归到 6 类任务：

- **需求建模**：把业务需求变成数据模型。
- **结构建模**：把数据库结构变成数据模型。
- **数据操作**：围绕已有模型做查询和增删改。
- **Skill 开发**：把已有模型的查询经验沉淀成可复用 Skill。
- **元素开发**：在 IDE 中创建、修改和修复 JitAI 元素。
- **业务洞察**：围绕开放业务问题形成分析报告。

## 六类内置 Agent 分别在做什么 {#built-in-agent-groups}
...
### 4. 问数 Skill 开发 Agent {#query-skill-development-agent}

`基于数据模型开发问数 Skill`面向开发者使用，适合“模型已经建好，希望把这个模型的常见查询、统计口径和 TQL 模板沉淀成可复用 Skill”的场景。
...
3. Agent 向`数据查询取数`子 Agent 提出模板生成和验证要求。
...
- 只是要查一次数据。应直接使用`数据查询取数`。
- 要回答开放经营问题。应使用`业务数据洞察分析`，必要时由它调用取数链路。

相关文档：

- [开发问数 Skill](../skills/query-skill-best-practices)
- [把 Skill 安装到 Agent](../skills/use-skill-in-agent)
- [Skill 创建最佳实践](../skills/skill-best-practices)

### 5. AI 开发助手 {#ai-development-assistant}

AI 开发助手面向开发者使用，适合在 IDE 中用自然语言创建、修改和修复 JitAI 元素。它可以辅助处理数据模型、服务函数、页面、Agent、Skill、Shell、事件、任务和数据库配置等开发任务。

它与业务 Agent 的区别很明确：

- 业务 Agent 面向最终用户，处理业务问答、数据查询和业务操作。
- AI 开发助手面向开发者，处理应用元素的创建和源码级修改。

使用 AI 开发助手时，开发者应先说明目标，再审阅差异、应用变更、构建刷新并验证结果。不要把源码工作空间或开发助手能力配置给普通业务 Agent。

完整使用方式见[使用 AI 开发助手开发 JitAI 元素](./ai-development-assistant)。

### 6. 业务洞察 Agent {#business-data-insight}
...
| [AI 开发助手](./ai-development-assistant) | 在 IDE 中辅助开发者创建和修改 JitAI 元素 |
