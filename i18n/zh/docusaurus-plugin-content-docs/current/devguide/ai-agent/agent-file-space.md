...
在 Agent 编辑器中进入 `运行与安全` -> `文件空间`，可配置当前 Agent 可访问的文件空间，以及组织空间是否允许当前 Agent 写入。
...
如果 Agent 同时启用了数据模型附件能力，可以在数据模型工具中把业务附件与文件空间中的文件关联起来。文件空间负责文件的读取、写入和路径管理；数据模型附件能力负责把文件挂到具体业务记录上。

:::warning
AI 开发助手使用的源码工作空间属于开发场景，不应作为普通业务 Agent 的文件空间配置。普通业务 Agent 只应访问完成业务任务所需的 Agent 空间、个人空间或组织空间。需要用自然语言创建或修改 JitAI 元素时，请使用[AI 开发助手](./ai-development-assistant)。
:::
