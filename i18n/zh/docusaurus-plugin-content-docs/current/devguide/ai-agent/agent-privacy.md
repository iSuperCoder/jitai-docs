...
在 Agent 编辑器中进入 `运行与安全` -> `隐私保护` 进行配置。
...
如果 Agent 会调用数据模型、服务函数、知识库或联网搜索，需要特别关注“工具结果再次进入模型前处理”。开启后，工具返回的敏感字段会先脱敏，再进入下一轮模型推理；关闭时，模型会看到工具原始结果。该配置只影响模型可见内容，不改变业务系统中的原始数据。
