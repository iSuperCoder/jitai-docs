---
sidebar_position: 5
sidebar_label: 为 Agent 安装 Skill
...
使用智能模式时，Skill 的名称和描述要写得足够清楚，否则 Agent 难以判断什么时候使用哪个 Skill。如何写清楚名称和描述，可参考[创建标准 Skill](./create-skill)。
...
如果效果不稳定，优先检查 Skill 的适用场景、能力范围和执行步骤是否清楚。设计和编写原则见[设计稳定可复用的 Skill](./skill-best-practices)。
