...
JitNode 安装包分为桌面版（支持Windows、Mac）和服务器版（支持Docker），一键安装JitNode，即可拥有所有基础设施，免去各种工具安装的技术复杂度。
...
我们正在积极开发以支持 Apple M 系列芯片。目前该版本对部分 Apple M 系列芯片的兼容性有限。如果您使用的是 Apple M 系列芯片设备，可以尝试安装使用。若遇到无法正常启动或激活页面无法打开的情况，敬请反馈并关注最新适配进展。
:::
